...
class DiscoverRepository {
  Future<Either<AppFailure, List<NearbySalonItem>>> getNearby({
    ...
    try {
      ...
      final response =
          await ApiClient.instance.get(uri).timeout(const Duration(seconds: 15));
  ...
  Future<Either<AppFailure, List<TopSalonItem>>> getTopSalons({
    ...
    try {
      final uri = Uri.parse('${ServerConstants.serverUrl}/search/top-salons')
  ...
  Future<Either<AppFailure, List<TrendingStyleItem>>> getTrending({
    ...
    try {
      final uri = Uri.parse('${ServerConstants.serverUrl}/search/trending')
          .replace(queryParameters: {'limit': limit.toString()});
      final response =
          await ApiClient.instance.get(uri).timeout(const Duration(seconds: 15));
      if (response.statusCode == 200) {
